...
from audio_analysis.audio_handler import AudioHandler, DemoAudioHandler
import joblib
...
def audio_thread():
    ...
    audio = DemoAudioHandler(60, svm, knn, './data/test/sounds.wav', event_monitor)
    # keep loading the file and running the loop every min for demo sake
    try:
       while True:
...
if __name__ == "__main__":
    ...
    cry_event_name = 'Crying Event'
    event_monitor.register_event(cry_event_name)
    ...
    sound_thread = threading.Thread(target=audio_thread, daemon=True)
    ...
    try:
        while True:
            ...
            if event_monitor.events[cry_event_name].is_set():
               event_monitor.set_event(cry_event_name)
               event_monitor.reset_event(cry_event_name)

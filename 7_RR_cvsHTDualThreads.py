...
class RespiratoryRateCalculator:
    ...
    def plot_data(self, data_to_plot, timestamps_to_plot):
        # Plot the raw data
        plt.figure(figsize=(12, 6))
        plt.subplot(2, 1, 1)  # Two subplots, this is the first
        plt.plot(timestamps_to_plot, data_to_plot, label='Raw Data')
        plt.xlabel('Time')
        ...
        data = np.array(data_to_plot)
        window = scipy.signal.hamming(len(data), sym=0)
        data *= window
        fft = np.fft.fft(data)
        fft = np.absolute(fft)
        fft = np.square(fft)
        sample_interval = (timestamps_to_plot[-1] - timestamps_to_plot[0]).total_seconds() / len(data_to_plot)
        frequencies = np.fft.fftfreq(len(data), d=sample_interval)

        plt.subplot(2, 1, 2)  # Two subplots, this is the second
        plt.plot(frequencies[:len(frequencies)//2], fft[:len(fft)//2], label='FFT Output')
...
try:
    while True:
        ...
        try:
            ...
            rr_calculator.plot_data(data_to_plot, timestamps_to_plot)
        except queue.Empty:

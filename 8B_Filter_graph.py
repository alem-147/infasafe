...
class RespiratoryRateCalculator:
    def __init__(self, buffer_size=300, start_size=100):
        ...
        self.fft_results = []  # Store FFT results
        self.fft_results_power = []
        self.fft_results_powerB = []
    ...
    def calculate_rr(self):
        ...
        filtered_data_raw = scipy.signal.filtfilt(b, a, data)
        self.test_buffer= filtered_data_raw.tolist()
        ...
        fftB = np.fft.fft(filtered_data_raw)
        fft_magnitudeB = np.absolute(fftB)
        fft_powerB = np.square(fft_magnitudeB)
    ...
    def plot_data(self, timestamps_to_plot):
        ...
        fft_powerB = self.fft_results_powerB[-1]
        fft = self.fft_results[-1]
        ...
        plt.plot(timestamps_to_plot, self.filtered_buffer, label='A Filtered Raw Data')
        plt.xlabel('Time')
        plt.ylabel('Normalized Value')
        plt.title('Normalized Data Over Time')
        plt.legend()
        plt.subplot(5, 1, 3)  # One subplot
        plt.plot(timestamps_to_plot, self.test_buffer, label='B Filtered normalized Data')
        plt.xlabel('Time')
        plt.ylabel('Temperature Value')
        plt.title('Filtered Temperature Data Over Time')
        plt.legend()
        plt.subplot(5, 1, 4)  # One subplot
        plt.plot(frequencies[:len(frequencies)//2], fft_power[:len(fft_power)//2], label='FFT power A Output')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Power')
        plt.title('FFT of Data')
        plt.legend()
        plt.subplot(5, 1, 5)  # One subplot
        plt.plot(frequencies[:len(frequencies)//2], fft_powerB[:len(fft_powerB)//2], label='FFT power B Output')
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('Power')
        plt.title('FFT of power Data')
        plt.legend()
        plt.tight_layout()
        plt.show()
...
try:
    while True:
        ...
        try:
            ...
            data_to_plot, timestamps_to_plot = plot_queue.get_nowait()
            # Plot the data using the main thread
            rr_calculator.plot_data(timestamps_to_plot)
        except queue.Empty:
